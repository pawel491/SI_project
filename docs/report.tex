...
\begin{table}[H]
    ...
    \begin{center}
        \begin{tabular}{|c|c|c|}
            ...
            \hline
            Random Forest & 97\% & 90.7\%\\
            SVM & 96\% & 86.4\%\\
            AdaBoost & 95\% &  86.5\%\\
            Naive Bayes & 80\% & 84.9\%\\
            Random Forest (Weights) & 78\% &89.2\% \\
            AdaBoost (Weights) & 76,6\% & 72.67\%\\
            \hline
        \end{tabular}
    \end{center}
    \label{SD_tab}
\end{table}
Table \ref{SD_tab} summarizes the accuracy and AUC metrics
for each tested model, highlighting their performance in
classifying the data accurately.The Random Forest model
has the highest wanted outputs percentage for test data.

\end{document}



%! below is nothing needed
% \section{Ease of Use}
